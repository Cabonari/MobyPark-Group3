...
class RequestHandler(BaseHTTPRequestHandler):
        ...
        if self.path == "/register":
            ...
            if "sessions" in self.path:
                ...
                if self.path.endswith("start"):
                    ...
                    filtered = {
                        ...
                        if value.get("licenseplate") == data["licenseplate"]
                        and not value.get("stopped")
                    }
                    if len(filtered) > 0:
                        self.send_response(401)
                        self.send_header("Content-type", "application/json")
            ...
            for field in ["licenseplate", "startdate", "enddate", "parkinglot"]:
                if not field in data:
                    ...
                    self.wfile.write(
                        json.dumps(
                            {"error": "Require field missing", "field": field}
                        ).encode("utf-8")
                    )
                    return
            if data.get("parkinglot", -1) not in parking_lots:
                ...
                self.wfile.write(
                    json.dumps(
                        {"error": "Parking lot not found", "field": "parkinglot"}
                    ).encode("utf-8")
                )
                return
            if "ADMIN" == session_user.get("role"):
                if not "user" in data:
                    ...
                    self.wfile.write(
                        json.dumps(
                            {"error": "Require field missing", "field": "user"}
                        ).encode("utf-8")
                    )
                    return
            else:
            ...
            self.end_headers()
            self.wfile.write(
                json.dumps({"status": "Success", "reservation": data}).encode("utf-8")
            )
            return
            ...
            if lid in uvehicles:
                ...
                self.wfile.write(
                    json.dumps(
                        {"error": "Vehicle already exists", "data": uvehicles.get(lid)}
                    ).encode("utf-8")
                )
                return
            if not uvehicles:
            ...
            self.end_headers()
            self.wfile.write(
                json.dumps({"status": "Success", "vehicle": data}).encode("utf-8")
            )
            return
            ...
            if lid not in uvehicles:
                ...
                self.end_headers()
                self.wfile.write(
                    json.dumps({"error": "Vehicle does not exist", "data": lid}).encode(
                        "utf-8"
                    )
                )
                return
            ...
            if self.path.endswith("/refund"):
                ...
                if "amount" not in data:
                    ...
                    self.wfile.write(
                        json.dumps(
                            {"error": "Require field missing", "field": "amount"}
                        ).encode("utf-8")
                    )
                    return
            ...
            self.end_headers()
            self.wfile.write(
                json.dumps({"status": "Success", "payment": payment}).encode("utf-8")
            )
            return
    ...
    def do_PUT(self):
        if self.path.startswith("/parking-lots/"):
            ...
            if rid:
                if rid in reservations:
                    ...
                    self.end_headers()
                    self.wfile.write(
                        json.dumps({"status": "Updated", "reservation": data}).encode(
                            "utf-8"
                        )
                    )
                    return
        ...
        # elif self.path.startswith("/payments/") and self.command == "PUT":
        #     token = self.headers.get("Authorization")
        #     session_user = get_session(token)
        #     if not token or not session_user:
        #         self.send_response(401)
        ...
        #     transaction_id = self.path.replace("/payments/", "")
        #     payments = load_payment_data()
        #     payment = next(
        #         (p for p in payments if p["transaction"] == transaction_id), None
        #     )
        ...
        #         self.send_header("Content-type", "application/json")
        #         self.end_headers()
        #         self.wfile.write(b'{"error": "Payment not found"}')
        #         return
        ...
        #     self.wfile.write(
        #         json.dumps(
        #             {"status": "Success", "payment": payment}, default=str
        #         ).encode("utf-8")
        #     )
        #     return
        elif self.path.startswith("/payments/") and self.command == "PUT":
            ...
            payment = next(
                (p for p in payments if p.get("transaction") == transaction_id), None
            )
            ...
            # Validate hash (IMPORTANT FIX)
            if payment.get("validation_hash") != data["validation"]:
                self.send_response(401)
                self.send_header("Content-type", "application/json")
                self.end_headers()
                self.wfile.write(
            ...
            if rid:
                if rid in reservations:
                    ...
                    if not token or not get_session(token):
                        ...
                        return
                    session_user = get_session(token)
                    if "ADMIN" == session_user.get("role") or session_user[
                        "username"
                    ] == reservations[rid].get("user"):
    ...
    def do_GET(self):
        ...
        if self.path == "/profile":
            ...
            if lid:
                ...
                if "sessions" in self.path:
                    ...
                    if self.path.endswith("/sessions"):
                        ...
                        sid = self.path.split("/")[-1]
                        if not "ADMIN" == session_user.get("role") and not session_user[
                            "username"
                        ] == sessions[sid].get("user"):
                            self.send_response(403)
                            self.send_header("Content-type", "application/json")
                            self.end_headers()
                            self.wfile.write(b"Access denied")
            ...
            payments = [p for p in load_payment_data() if p.get("initiator") == user]
            ...
            for pid, parkinglot in load_parking_lot_data().items():
                for sid, session in load_json(
                    f"data/pdata/p{pid}-sessions.json"
                ).items():
                    if session["user"] == session_user["username"]:
                        ...
                        data.append(
                            {
                                "session": {
                                    ...
                                    if k in ["licenseplate", "started", "stopped"]
                                }
                                | {"hours": hours, "days": days},
                                "parking": {
                                    k: v
                                    for k, v in parkinglot.items()
                                    if k in ["name", "location", "tariff", "daytariff"]
                                },
                                "amount": amount,
                                "thash": transaction,
                                "payed": payed,
            ...
            if self.path.endswith("/reservations"):
                ...
                self.end_headers()
                self.wfile.write(
                    json.dumps(vehicles.get(user, {}), default=str).encode("utf-8")
                )
                return
